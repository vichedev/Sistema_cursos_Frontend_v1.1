          ...
          <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-xl border border-gray-200 dark:border-gray-600 transition-colors duration-200">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
              {isExpired
                ...
                : "Este curso fue archivado por el administrador."}
            </p>
          </div>
        </div>
...
const EmptyState = ({ activeTab, searchTerm, cursosCount }) => {
  const getEmptyMessage = () => {
    ...
    const messages = {
      ACTIVOS: { title: "No tienes cursos activos", description: "Todos tus cursos están inactivos o archivados" },
      INACTIVOS: { title: "No tienes cursos inactivos", description: "Todos tus cursos están activos" },
      GRATIS: { title: "No tienes cursos gratuitos", description: "Solo tienes cursos premium" },
      PAGADOS: { title: "No tienes cursos pagados", description: "Solo tienes cursos gratuitos" }
    };

    return messages[activeTab] || { title: "No hay cursos", description: "No se encontraron cursos" };
  };
...
export default function MisCursos() {
  ...
  const statistics = useMemo(() => {
    const cursosActivos = misCursos.filter((c) => c.activo && !isCourseExpired(c));
    const cursosInactivos = misCursos.filter((c) => !c.activo || isCourseExpired(c));
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-4 md:p-5 mb-6 transition-colors duration-200">
            ...
            <div className="grid grid-cols-4 gap-2 w-full">
              <div className="flex flex-col items-center justify-center p-1">
                ...
                <div className="bg-white dark:bg-gray-700 p-1 rounded-lg text-center w-full transition-colors duration-200">
                  <div className="font-bold text-blue-600 dark:text-blue-400">{statistics.total}</div>
                </div>
              </div>
              <div className="flex flex-col items-center justify-center p-1">
                <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Activos</div>
                <div className="bg-white dark:bg-gray-700 p-1 rounded-lg text-center w-full transition-colors duration-200">
                  <div className="font-bold text-green-600 dark:text-green-400">{statistics.cursosActivos.length}</div>
                </div>
              </div>
              <div className="flex flex-col items-center justify-center p-1">
                <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Inactivos</div>
                <div className="bg-white dark:bg-gray-700 p-1 rounded-lg text-center w-full transition-colors duration-200">
                  <div className="font-bold text-blue-600 dark:text-blue-400">{statistics.cursosInactivos.length}</div>
                </div>
              </div>
              <div className="flex flex-col items-center justify-center p-1">
                <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Pagados</div>
                <div className="bg-white dark:bg-gray-700 p-1 rounded-lg text-center w-full transition-colors duration-200">
                  <div className="font-bold text-purple-600 dark:text-purple-400">{statistics.cursosPagados.length}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
            {[
              { key: "ACTIVOS", label: "Activos", icon: FaGraduationCap, count: statistics.cursosActivos.length },
              { key: "INACTIVOS", label: "Inactivos", icon: FaMoneyBillWave, count: statistics.cursosInactivos.length },
              { key: "GRATIS", label: "Gratis", icon: FaFilter, count: statistics.cursosGratis.length },
              { key: "PAGADOS", label: "Pagados", icon: FaMoneyBillWave, count: statistics.cursosPagados.length }
            ].map((tab) => (
              <button
                key={tab.key}
